...
export default function CheckInPage() {
  ...
  const handleCheckIn = async (guestId: string) => {
    const supabase = createClient();
    const { error } = await supabase.from("checkins").insert({ event_id: eventId, guest_id: guestId, method: "manual", synced: navigator.onLine });
  ...
  const handleQR = () => {
    setScanResult({ type: "success", guest: guests.find((g) => g.rsvp_status === "yes") });
    setTimeout(() => setScanResult(null), 3000);
  };
  ...
  return (
    <div>
      <Link href={"/events/" + eventId} className="inline-flex items-center gap-1 text-[13px] text-[#6C757D] hover:text-[#4338CA] mb-4"><ArrowLeft size={14} /> Event</Link>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-[26px] font-semibold text-[#212529] font-[family-name:var(--font-display)] tracking-[-0.8px]">Check-In</h1>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <Card padding="sm" variant="elevated"><p className="text-[12px] text-[#6C757D]">Checked In</p><p className="text-[24px] font-bold text-[#22C55E]">{stats.checked}</p></Card>
                  ...
                  {filtered.map((guest) => {
                    ...
                    return (
                      <div key={guest.id} className={"flex items-center justify-between p-3 rounded-[8px] border-[0.5px] " + (checked ? "bg-[#EAF7EF] border-[#BBE8CC]" : "bg-white border-[#DEE2E6]")}>
                        ...
                        {checked ? <Badge variant="checkedIn">Checked In</Badge> : <Button variant="primary" size="sm" onClick={() => handleCheckIn(guest.id)}>Check In</Button>}
                      </div>
                    );
                  })}
            ...
            {activeTab === "walkin" && (
              <Card>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <Select label="Category" options={["vip","regular","family","friends","corporate","media","other"].map((c) => ({ value: c, label: c.charAt(0).toUpperCase() + c.slice(1) }))} value={walkInCategory} onChange={(e) => setWalkInCategory(e.target.value as GuestCategory)} />
